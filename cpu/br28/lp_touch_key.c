...
#ifndef TCFG_LP_TOUCH_CHECK_LONG_BY_RES_ENABLE
#define TCFG_LP_TOUCH_CHECK_LONG_BY_RES_ENABLE    1
#endif

#define CTMU_CHECK_LONG_CLICK_BY_RES    TCFG_LP_TOUCH_CHECK_LONG_BY_RES_ENABLE
...
#ifndef TCFG_LP_TOUCH_ALOG_RANGE_MIN
#define TCFG_LP_TOUCH_ALOG_RANGE_MIN 50
#endif

#ifndef TCFG_LP_TOUCH_ALOG_RANGE_MAX
#define TCFG_LP_TOUCH_ALOG_RANGE_MAX 500
#endif

#define TOUCH_RANGE_MIN     TCFG_LP_TOUCH_ALOG_RANGE_MIN
#define TOUCH_RANGE_MAX     TCFG_LP_TOUCH_ALOG_RANGE_MAX
...
#if TCFG_LP_TOUCH_PB1_LED_FEEDBACK_ENABLE
    /* PB1 (CH1 = eartch primary) is GPIO-only feedback.
     * FALLING/RAISING still fire (driving PC3 high/low), but
...
#ifndef TCFG_LP_EARTCH_REF_KEY_EVENT_ENABLE
#define TCFG_LP_EARTCH_REF_KEY_EVENT_ENABLE 0
#endif

#ifndef TCFG_LP_EARTCH_LOGIC_INVERT
#define TCFG_LP_EARTCH_LOGIC_INVERT 0
#endif

#if TCFG_LP_EARTCH_LOGIC_INVERT
#define LP_EARTCH_EVENT_IN_STATE    EAR_OUT
#define LP_EARTCH_EVENT_OUT_STATE   EAR_IN
#else
#define LP_EARTCH_EVENT_IN_STATE    EAR_IN
#define LP_EARTCH_EVENT_OUT_STATE   EAR_OUT
#endif
    ...
    for (u8 ch = 0; ch < LP_CTMU_CHANNEL_SIZE; ch ++) {
        if (__this->config->ch[ch].enable) {
            ...
            if (__this->config->eartch_en && (__this->config->eartch_ch == ch)) {
                /* FIX-016 BLOCK1: Only set EARTCH hardware mode when NOT using CH1 as a key.
                 * With EARTCH hw mode set (BIT1), P33 sends EARTCH_IN/OUT events ONLY - no
                 * RAISING/FALLING/SHORT/LONG events at all. CH3 works because it never
                 * gets this bit set. */
#if !TCFG_LP_EARTCH_PRIMARY_KEY_EVENT_ENABLE
                ...
                //软件触摸灵敏度调试
                // FIXED: Swap thresholds - when touch (low resistance), it's IN_EAR, so lower threshold = inear
...
#if TCFG_LP_EARTCH_PRIMARY_KEY_EVENT_ENABLE
                /* FIX-016 BLOCK2: Also init CH1 as a normal key channel (like CH3).
                 * Without this, alog_cfg[1].ready_flag stays 0 forever, so
                 * lp_touch_key_ctmu_res_scan always skips CH1 - no adaptive cfg2
                 * adjustment, no TouchAlgo running. CH3 gets this because it goes
                 * through the else branch below. CH1 must also get it. */
#if !TCFG_LP_TOUCH_KEY_BT_TOOL_ENABLE
                TouchAlgo_Init(ch, TOUCH_RANGE_MIN, TOUCH_RANGE_MAX);
                alog_sta_cnt[ch] = 0;
                ch_range_sensity[ch] = ch_sensity;
                log_info("CH1 key init: read vm alog cfg\n");
                {
                    int alog_ret = syscfg_read(VM_LP_TOUCH_KEY0_ALOG_CFG + ch, (void *)&alog_cfg[ch], sizeof(struct lp_touch_key_alog_cfg));
                    if ((alog_ret == (sizeof(struct lp_touch_key_alog_cfg))) && (alog_cfg[ch].range > TOUCH_RANGE_MIN) && (alog_cfg[ch].range < TOUCH_RANGE_MAX)) {
                        log_info("CH1 key: vm alog ready:%d sigma:%d range:%d\n", alog_cfg[ch].ready_flag, alog_cfg[ch].sigma, alog_cfg[ch].range);
                        u16 cfg0k = (M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG0H + ch * 8) << 8) | (M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG0L + ch * 8));
                        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG1L + ch * 8) = (cfg0k + 5) & 0xff;
                        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG1H + ch * 8) = ((cfg0k + 5) >> 8) & 0xff;
                        u16 cfg2k_new = alog_cfg[ch].range * (10 - ch_range_sensity[ch]) / 10;
                        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG2L + ch * 8) = cfg2k_new & 0xff;
                        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG2H + ch * 8) = (cfg2k_new >> 8) & 0xff;
...
void p33_ctmu_key_event_irq_handler()
{
        ...
        if (__this->config->eartch_en && (__this->config->eartch_ch == ch_num)) {
            ...
            if (P2M_CTMU_EARTCH_EVENT != last_state) {
                ...
                if (last_state == CTMU_P2M_EARTCH_IN_EVENT) {
                    ...
                    lpctmu_tws_send_event_data(LP_EARTCH_EVENT_IN_STATE, BT_EVENT_SW_MSG);
#endif
                    ...
                    printf("soft outear");
#if TWS_BT_SEND_EVENT_ENABLE
                    lpctmu_tws_send_event_data(LP_EARTCH_EVENT_OUT_STATE, BT_EVENT_SW_MSG);
#endif
                    if (__this->eartch_inear_ok) {
...
#if TCFG_LP_EARTCH_KEY_ENABLE
        // Single-touch fallback: press on primary ear channel is treated as IN-EAR - only if not configured to generate key events.
#if !TCFG_LP_EARTCH_PRIMARY_KEY_EVENT_ENABLE
        if (__this->config->eartch_en && (__this->config->eartch_ch == ch_num)) {
            if (__this->eartch_inear_ok) {
                ctmu_eartch_event_handle(LP_EARTCH_EVENT_IN_STATE);
    ...
    case CTMU_P2M_CH4_RAISING_EVENT:
         ...
         * through to ctmu_raise_click_handle() exactly like CH3. */
#if !TCFG_LP_EARTCH_PRIMARY_KEY_EVENT_ENABLE
        if (__this->config->eartch_en && (__this->config->eartch_ch == ch_num)) {
            if (__this->eartch_inear_ok) {
                ctmu_eartch_event_handle(LP_EARTCH_EVENT_OUT_STATE);
