...
// Ensure the Linux download tools are executable (permissions are lost on clone/copy)
...
// strip-ini removes comment/blank lines from isd_config.ini before flashing.
// /opt/utils/strip-ini is a Jieli tool that may be absent; sed does the same job.
