...
// Manual MAC provisioning switch must be defined before cfg_file_parse() uses it.
#define TCFG_MANUAL_MAC_PROVISIONING_ENABLE  1
...
static u8 manual_cfg_mac_override[6] = {0};
static u8 manual_cfg_mac_override_valid = 0;
...
void cfg_file_parse(u8 idx)
{
    ...
    if (ret < 0) {
        memcpy(bt_cfg.ble_name, bt_cfg.edr_name, LOCAL_NAME_LEN);
    } else if (ret >= LOCAL_NAME_LEN) {
    ...
    log_info("ble name config:%s", bt_get_ble_local_name());
    ...
    if (ret == sizeof(aec)) {
        ...
        app_var.aec_mic2_gain = 8;  /* was 1 — too low for ANC feedback error mic; 8 gives adequate loop gain */
...
#if TCFG_MANUAL_MAC_PROVISIONING_ENABLE
    // Enforce manual provisioning in current runtime even if BTIF still returns old/random data.
    if (manual_cfg_mac_override_valid) {
        memcpy(mac_buf, manual_cfg_mac_override, 6);
        syscfg_write(CFG_BT_MAC_ADDR, mac_buf, 6);
    }
#endif
...
//======================================================================================//
// Pair 1 Configuration (P1)
// Left Bud (B01):  EDR MAC = 3C:00:0A:7E:1A:00, TWS Local = 3C:00:0A:7E:1A:00
// Right Bud (B02): EDR MAC = 3C:00:0A:7E:1A:01, TWS Local = 3C:00:0A:7E:1A:01
// Sibling relationship: B01 TWS_REMOTE = B02 TWS_LOCAL and vice versa
// Pair Code: 0x6688 (prevents cross-pair interference)

#if TCFG_MANUAL_MAC_PROVISIONING_ENABLE

// Helper function: Provision manual MAC addresses for Left Bud (Pair 1, B01)
void bt_provision_pair1_left_bud(void)
{
    u8 edr_mac[6]        = P1_COMMON_MAC;  // shared EDR MAC — phone pairs with this on either bud
    u8 tws_local[6]      = P1_LEFT_MAC;    // unique TWS identity for internal sibling link
    u8 tws_remote[6]     = P1_RIGHT_MAC;
    u8 tws_common[6]     = P1_COMMON_MAC;
    u8 tws_channel       = BUD_LEFT;
    u16 pair_code        = P1_PAIR_CODE;

    log_info("Provisioning Pair 1 - LEFT BUD (B01) [shared EDR MAC]");

    syscfg_write(CFG_BT_NAME, (u8 *)BT_DEVICE_NAME_STR, sizeof(BT_DEVICE_NAME_STR));
    memset(bt_cfg.edr_name, 0, LOCAL_NAME_LEN);
    memcpy(bt_cfg.edr_name, BT_DEVICE_NAME_STR, sizeof(BT_DEVICE_NAME_STR));
    log_info("  CFG_BT_NAME (101) = " BT_DEVICE_NAME_STR);

    syscfg_write(CFG_BT_MAC_ADDR, edr_mac, 6);
    log_info("  CFG_BT_MAC_ADDR (102) = AA:BB:CC:00:01:FF [shared]");

    syscfg_write(CFG_TWS_LOCAL_ADDR, tws_local, 6);
    log_info("  CFG_TWS_LOCAL_ADDR (95) = 3C:00:0A:7E:1A:00 [left unique]");
    
    syscfg_write(CFG_TWS_REMOTE_ADDR, tws_remote, 6);
    log_info("  CFG_TWS_REMOTE_ADDR (96) = 3C:00:0A:7E:1A:01 [sibling]");
    
    syscfg_write(CFG_TWS_COMMON_ADDR, tws_common, 6);
    log_info("  CFG_TWS_COMMON_ADDR (97) = AA:BB:CC:00:01:FF [pair]");
    
    syscfg_write(CFG_TWS_CHANNEL, &tws_channel, 1);
    log_info("  CFG_TWS_CHANNEL (98) = 0x00 [LEFT]");
    
    syscfg_write(CFG_TWS_PAIR_CODE_ID, (u8 *)&pair_code, 2);
    log_info("  CFG_TWS_PAIR_CODE_ID (602) = 0x6688");

    memcpy(manual_cfg_mac_override, edr_mac, 6);
    manual_cfg_mac_override_valid = 1;
}

// Helper function: Provision manual MAC addresses for Right Bud (Pair 1, B02)
void bt_provision_pair1_right_bud(void)
{
    u8 edr_mac[6]        = P1_COMMON_MAC;  // shared EDR MAC — phone pairs with this on either bud
    u8 tws_local[6]      = P1_RIGHT_MAC;   // unique TWS identity for internal sibling link
    u8 tws_remote[6]     = P1_LEFT_MAC;
    u8 tws_common[6]     = P1_COMMON_MAC;
    u8 tws_channel       = BUD_RIGHT;
    u16 pair_code        = P1_PAIR_CODE;

    log_info("Provisioning Pair 1 - RIGHT BUD (B02) [shared EDR MAC]");
    ...
    syscfg_write(CFG_TWS_LOCAL_ADDR, tws_local, 6);
    log_info("  CFG_TWS_LOCAL_ADDR (95) = 3C:00:0A:7E:1A:01 [right unique]");
    
    syscfg_write(CFG_TWS_REMOTE_ADDR, tws_remote, 6);
    log_info("  CFG_TWS_REMOTE_ADDR (96) = 3C:00:0A:7E:1A:00 [sibling]");
    
    syscfg_write(CFG_TWS_COMMON_ADDR, tws_common, 6);
    log_info("  CFG_TWS_COMMON_ADDR (97) = AA:BB:CC:00:01:FF [pair]");
    
    syscfg_write(CFG_TWS_CHANNEL, &tws_channel, 1);
    log_info("  CFG_TWS_CHANNEL (98) = 0x01 [RIGHT]");
...
#endif  // TCFG_MANUAL_MAC_PROVISIONING_ENABLE
