...
//PB1 Touch to LED Feedback Configuration
#if TCFG_LP_TOUCH_PB1_LED_FEEDBACK_ENABLE
static u8 pb1_led_initialized = 0;

static void pb1_touch_led_init(void)
{
    if (pb1_led_initialized) {
        return;
    }
    // Initialize PC3 as GPIO output for LED feedback
    gpio_direction_output(TCFG_LP_TOUCH_PB1_LED_PORT, !TCFG_LP_TOUCH_PB1_LED_ACTIVE_LEVEL);
    pb1_led_initialized = 1;
    log_info("PB1 Touch LED Feedback initialized on PC3");
}

static void pb1_touch_led_control(u8 on_off)
{
    pb1_touch_led_init();
    u8 output_level = on_off ? TCFG_LP_TOUCH_PB1_LED_ACTIVE_LEVEL : !TCFG_LP_TOUCH_PB1_LED_ACTIVE_LEVEL;
    gpio_write(TCFG_LP_TOUCH_PB1_LED_PORT, output_level);
}
#endif  // TCFG_LP_TOUCH_PB1_LED_FEEDBACK_ENABLE

// Public wrapper function to initialize PB1 touch LED feedback
...
static void lr_diff_otp_deal(u8 opt, char channel)
{
    ...
    case ONE_KEY_CTL_VOL_UP_DOWN:
        // Right bud -> Vol Up, Left bud -> Vol Down
        if (channel == 'R') {
            volume_up(1);
        } else if (channel == 'L') {
            volume_down(1);
...
int app_earphone_key_event_handler(struct sys_event *event)
{
...
#if TCFG_LP_TOUCH_PB1_LED_FEEDBACK_ENABLE
    /* PB1 (ch1, key_value=2): touch → PC3 LOW, release → PC3 HIGH.
     * HOLD fires every 200ms reasserting LOW while finger stays on pad. */
    if (key->value == 2) {
        gpio_direction_output(TCFG_LP_TOUCH_PB1_LED_PORT,
                              key->event == KEY_EVENT_UP ? 1 : 0);
    }
#endif
    ...
    case  KEY_MUSIC_NEXT:
        ...
        if (key->value == 2 && get_bt_tws_connect_status()) {
            // KEY_2 double-tap: Right bud -> Next, Left bud -> Prev
