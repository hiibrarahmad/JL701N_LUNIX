...
//                                                                                 //
//  ── DEVICE NAME ─────────────────────────────────────────────────────────────  //
//  BT_DEVICE_NAME : Bluetooth name shown on phone. Max 31 chars.                 //
//  Both buds of a pair should use the same name (phone sees one device).         //
//                                                                                 //
//  ── BUD ROLE ────────────────────────────────────────────────────────────────  //
//  THIS_BUD:  BUD_RIGHT → Hold=VolUp   / DoubleTap=Next  / EDR = B02 MAC        //
//             BUD_LEFT  → Hold=VolDown / DoubleTap=Prev  / EDR = B01 MAC        //
//                                                                                 //
//  ── PAIR 1 MAC TABLE ────────────────────────────────────────────────────────  //
//    B01 Left  EDR/TWS-local : 3C:00:0A:7E:1A:00                                //
//    B02 Right EDR/TWS-local : 3C:00:0A:7E:1A:01                                //
//    Common pair address     : AA:BB:CC:00:01:FF  (unique per pair)              //
//    TWS discovery pair code : 0x6688                                            //
//                                                                                 //
//  NOTE: Phone connects to master bud EDR MAC only. Both buds in a pair          //
//  must be used together — the secondary bud connects via TWS, not phone.        //
//                                                                                 //
...
// In-ear touch master switch:
// ENABLE_THIS_MOUDLE  -> enable LP touch in-ear detection + event handling
// DISABLE_THIS_MOUDLE -> disable LP touch in-ear detection in one place
#define TCFG_INEAR_TOUCH_MASTER_ENABLE      ENABLE_THIS_MOUDLE

#define TCFG_LP_TOUCH_KEY0_EN    			0                  		//是否使能触摸按键0 —— PB0
#define TCFG_LP_TOUCH_KEY1_EN    			1                  		//是否使能触摸按键1 —— PB1 (in-ear primary)
#define TCFG_LP_TOUCH_KEY2_EN    			0                  		//是否使能触摸按键2 —— PB2
#define TCFG_LP_TOUCH_KEY3_EN    			1                  		//是否使能触摸按键3 —— PB4 (in-ear reference)
...
#if (TCFG_INEAR_TOUCH_MASTER_ENABLE == ENABLE_THIS_MOUDLE)
#define TCFG_LP_EARTCH_KEY_ENABLE 			1	 	                //是否使能触摸按键用作入耳检测 (需要2个通道)
#else
#define TCFG_LP_EARTCH_KEY_ENABLE 			0
#endif
#define TCFG_LP_EARTCH_KEY_CH               1                       //带入耳检测流程的按键号：PB1 (primary, ch1)
#define TCFG_LP_EARTCH_KEY_REF_CH           3                       //入耳检测算法需要的另一个按键通道作为参考：PB4 (reference, ch3)
#define TCFG_LP_EARTCH_SOFT_INEAR_VAL       1500                    //入耳检测算法需要的入耳阈值，降低值以适应PCB特性
#define TCFG_LP_EARTCH_SOFT_OUTEAR_VAL      800                     //出耳检测算法需要的出耳阈值，降低值以提高灵敏度
// Allow key events on PB1 even when PB1 is used as eartch primary channel (enables LED feedback on touch)
...
#define TCFG_LP_TOUCH_PB1_LED_ACTIVE_LEVEL      1                   //LED active level (1=high, 0=low)
...
#endif
#define TCFG_EARTCH_EVENT_HANDLE_ENABLE 	TCFG_INEAR_TOUCH_MASTER_ENABLE 		//入耳检测事件APP流程处理使能

// Set to 0 to ignore saved VM switch state and always use default-on behavior
// from eartch_event_deal.c (avoids accidental persistent disable by key command).
#define TCFG_EARTCH_SWITCH_CFG_ENABLE       0
...
#define TCFG_EAR_DETECT_CTL_KEY            	   	0                          //关闭入耳按键门控，避免屏蔽LP触摸按键事件
...
/*注意:ANC使能情况下，使用差分mic*/
/*JL7016G: MIC0(call/talk) is single-ended on PA1 only -> use CAP_MODE; MIC1/MIC2 are ANC mics, keep DIFF*/
...
#define TCFG_MPU6887P_AD0_SELETE_IO               NO_CONFIG_PORT //iic地址选择io
//icm42670p cfg
...
//                                  系统配置                                         //
//*********************************************************************************//
#define TCFG_AUTO_SHUT_DOWN_TIME		          600   //没有蓝牙连接自动关机时间 (10 min)
...
#define TCFG_BT_SUPPORT_LDAC                      1   //LDAC格式支持
...
#if (DUEROS_DMA_EN || TRANS_DATA_EN || TUYA_DEMO_EN || LL_SYNC_EN || RCSP_ADV_EN || RCSP_BTMATE_EN || SMART_BOX_EN)
...
#define TCFG_HW_SPI1_PORT_DO                IO_PORTG_05//IO_PORTG_05
#define TCFG_HW_SPI1_PORT_DI                IO_PORTG_06//IO_PORTG_06
// #define TCFG_HW_SPI1_PORT_INT               IO_PORTG_02//IO_PORTC_02
#define TCFG_HW_SPI1_BAUD                   8000000L
