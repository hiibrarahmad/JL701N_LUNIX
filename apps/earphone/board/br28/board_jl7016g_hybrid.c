...
u8 key_table[KEY_NUM_MAX][KEY_EVENT_MAX] = {
    ...
    // KEY_POWEROFF and KEY_POWEROFF_HOLD both removed from KEY_0:
    // PB1 is the in-ear capacitive sensor. Ear contact holds the sensor active continuously.
    // LONG (2s) fired KEY_POWEROFF → soft-poweroff while wearing.
    // HOLD (every 240ms) fired KEY_POWEROFF_HOLD → same restart loop.
    // CTMU hardware reset (8s) also removed via CTMU_RESET_TIME_CONFIG=0 in lp_touch_key_api.h.
    {KEY_MUSIC_PP,   KEY_NULL,        KEY_NULL,           KEY_NULL,     KEY_CALL_LAST_NO,     KEY_ANC_SWITCH},   //KEY_0
    {KEY_MUSIC_NEXT, KEY_VOL_UP,      KEY_VOL_UP,         KEY_NULL,     KEY_OPEN_SIRI,        KEY_NULL},   //KEY_1
    // KEY_2 is PB4 (LP touch ch3, key_value = 2)
    // Hold/Double are channel-aware: Right bud=VolUp/Next, Left bud=VolDown/Prev
    // (differentiated at runtime via key_tws_lr_diff_deal -- no per-bud firmware needed)
...
/* BOAT_PL在softoff的时候也保持高,不然会触发模块断电 */
    ...
    // Guard: only protect PC3 (external touch power port) if external touch IC is actually enabled.
    // Without the guard, this fired unconditionally and interfered with the PC3 LED feedback
    // (FIX-019) during the soft-power-off sequence triggered by KEY_POWEROFF_HOLD.
#if TCFG_EXTERNAL_TOUCH_KEY_ENABLE
